...
{
    sqlite3* DB;
    std::string sql1 ="DROP TABLE IF EXISTS ECXTTABLE;";
    std::string sql2 ="CREATE TABLE ECXTTABLE(TAG TEXT PRIMARY KEY,SIZE INTEGER);";    
    int exit = 0;
    char* messaggeError;

    exit = sqlite3_open("EDBCXT.db", &DB);
    if (exit != SQLITE_OK)
    {
        std::cerr << "Open database error" << std::endl;
        sqlite3_free(messaggeError);
...
void  eInsertCxt(char* tag,int tsizes, int cxtsize)
{
    char* zErrMsg = 0; // Error message var
    const char* sql = "INSERT INTO ECXTTABLE(TAG ,SIZE) VALUES (?,?)";
    sqlite3_stmt* stmt=NULL;
    int exit = 0;
    sqlite3* DB;
    exit = sqlite3_open("EDBCXT.db", &DB);
